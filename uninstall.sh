...
install_from_archive() {
  ...
  if [[ -z "${extracted}" || ! -f "${extracted}/scripts/install.py" ]]; then
    rm -rf "${tmp_dir}"
    return 1
  fi
  rm -rf "${INSTALL_DIR}"
...
resolve_repo_dir() {
  ...
  if [[ -f "${INSTALL_DIR}/scripts/install.py" ]]; then
...
python3 "${REPO_DIR}/scripts/install.py" --repo-dir "${REPO_DIR}" uninstall
