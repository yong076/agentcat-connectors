from ._base import HomeSpec, ProviderSpec, cost as _cost, discover as _discover, health as _health, quota as _quota, usage as _usage

SPEC = ProviderSpec(
    id="claude", display_name="Claude", brand_color="#D97757", icon_hint="claude",
    windows=("5h", "7d", "monthly", "session"), source_hint_key="provider.source.claude",
    homes=(HomeSpec("CLAUDE_CONFIG_DIR", ".claude", ("projects", "settings.json"),
                    ("projects/**/*.jsonl",), ".claude*"),),
    capabilities=("limits.claude.oauthUsage", "limits.claude.statusline", "limits.claude.statuslineQuotas",
                  "hooks.claudeStatusline", "hooks.claudeHooks"),
)

def discover(ctx): return _discover(ctx, SPEC)
def usage(ctx, home=None): return _usage(ctx, SPEC, home)
def quota(ctx, home=None): return _quota(ctx, SPEC, home)
def cost(ctx, usage_slice): return _cost(ctx, SPEC, usage_slice)
def health(ctx): return _health(ctx, SPEC)
